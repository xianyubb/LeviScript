// AUTO-GENERATED by tools/autobind/autobind.py - DO NOT EDIT.
// Regenerate from the JSON spec instead of modifying this file.


#include <memory>
#include <string>
#include <utility>
#include <vector>

#include "ll/api/event/player/PlayerDestroyBlockEvent.h"
#include "ll/api/event/Cancellable.h"
#include "mc/world/level/BlockPos.h"
#include "mc/deps/nbt/CompoundTag.h"
#include "ll/api/event/player/PlayerClickEvent.h"
#include "script/Local.h"
#include "script/ScriptEngine.h"
#include "script/bind/Bind.h"

LS_NATIVE_CLASS(::ll::event::Cancellable<ll::event::PlayerLeftClickEvent>)
LS_NATIVE_CLASS(::ll::event::player::PlayerDestroyBlockEvent)
LS_NATIVE_CLASS(::BlockPos)
LS_NATIVE_CLASS(::CompoundTag)
LS_NATIVE_CLASS(::ll::event::PlayerLeftClickEvent)

namespace ls::native::generated {

using ls::script::ClassBinder;
using ls::script::getGlobal;
using ls::script::Local;
using ls::script::makeFunction;
using ls::script::makeNativeFunction;
using ls::script::makeObject;
using ls::script::Object;
using ls::script::ScriptEngine;
using ls::script::Value;

void bind_ll_api_event_player_PlayerDestroyBlockEvent(ScriptEngine& engine) {
    Local<Object> global = getGlobal(engine);
    // Walk/create the (possibly nested, e.g. "ll.data") namespace path,
    // merging into existing objects so generated bindings compose with
    // hand-written ones instead of clobbering them.
    Local<Value>  probe0 = global.getProperty("ll");
    Local<Object> ns0 = probe0.isObject() ? Local<Object>(probe0) : makeObject(engine);
    Local<Value>  probe1 = ns0.getProperty("event");
    Local<Object> ns1 = probe1.isObject() ? Local<Object>(probe1) : makeObject(engine);
    Local<Value>  probe2 = ns1.getProperty("player");
    Local<Object> ns = probe2.isObject() ? Local<Object>(probe2) : makeObject(engine);

    // -- CancellablePlayerLeftClickEvent : PlayerLeftClickEvent --------------------
    ClassBinder::registerClass<::ll::event::Cancellable<ll::event::PlayerLeftClickEvent>, ::ll::event::PlayerLeftClickEvent>(engine, "CancellablePlayerLeftClickEvent");
    ClassBinder::method<::ll::event::Cancellable<ll::event::PlayerLeftClickEvent>>(engine, "serialize", &::ll::event::Cancellable<ll::event::PlayerLeftClickEvent>::serialize);
    ClassBinder::method<::ll::event::Cancellable<ll::event::PlayerLeftClickEvent>>(engine, "deserialize", &::ll::event::Cancellable<ll::event::PlayerLeftClickEvent>::deserialize);
    ClassBinder::method<::ll::event::Cancellable<ll::event::PlayerLeftClickEvent>>(engine, "isCancelled", &::ll::event::Cancellable<ll::event::PlayerLeftClickEvent>::isCancelled);
    ClassBinder::method<::ll::event::Cancellable<ll::event::PlayerLeftClickEvent>>(engine, "setCancelled", &::ll::event::Cancellable<ll::event::PlayerLeftClickEvent>::setCancelled);
    ClassBinder::method<::ll::event::Cancellable<ll::event::PlayerLeftClickEvent>>(engine, "cancel", &::ll::event::Cancellable<ll::event::PlayerLeftClickEvent>::cancel);
    ClassBinder::expose<::ll::event::Cancellable<ll::event::PlayerLeftClickEvent>>(engine, ns.handle(), "CancellablePlayerLeftClickEvent");

    // -- PlayerDestroyBlockEvent : CancellablePlayerLeftClickEvent --------------------
    ClassBinder::registerClass<::ll::event::player::PlayerDestroyBlockEvent, ::ll::event::Cancellable<ll::event::PlayerLeftClickEvent>>(engine, "PlayerDestroyBlockEvent");
    ClassBinder::method<::ll::event::player::PlayerDestroyBlockEvent>(engine, "serialize", &::ll::event::player::PlayerDestroyBlockEvent::serialize);
    ClassBinder::method<::ll::event::player::PlayerDestroyBlockEvent>(engine, "pos", &::ll::event::player::PlayerDestroyBlockEvent::pos);
    ClassBinder::expose<::ll::event::player::PlayerDestroyBlockEvent>(engine, ns.handle(), "PlayerDestroyBlockEvent");

    ns1.setProperty("player", ns);
    ns0.setProperty("event", ns1);
    global.setProperty("ll", ns0);
}

} // namespace ls::native::generated
